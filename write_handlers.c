...
*Return: the number f chars printed
*/
int handle_write_char(char c, char buffer[],
		int flags, int width, int precision, int size)
{/* char is stored at the left and paddind at buffer's right */
	int i = 0;
	char padd = ' ';
		...
		int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == ' ')/* extra char on left*/
		{
			if (extra_c)
				buffer[--ind] = extra_c;
...
/**
*write_unsgnd -writes an unisgned number
*@is_negative: Number indicating if the num is negative
*@ind: index in the buffer
*@buffer: Array of chars
*@flags: Flags specifiers
*@width: Width specifiees
*@precision: Precision specifier
*@size: Size specifier
*
...
*@ind: Index at which the number strats in the buffer
